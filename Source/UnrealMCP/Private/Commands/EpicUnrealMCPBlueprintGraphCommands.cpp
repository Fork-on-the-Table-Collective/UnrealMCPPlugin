...
TSharedPtr<FJsonObject> FEpicUnrealMCPBlueprintGraphCommands::HandleCommand(const FString& CommandType, const TSharedPtr<FJsonObject>& Params)
{
    ...
    {
        return FLocalVariableManager::AddLocalVariable(Params);
    }
    else if (CommandType == TEXT("get_local_variables"))
    {
        return FLocalVariableManager::GetLocalVariables(Params);
    }
    else if (CommandType == TEXT("remove_local_variable"))
    {
        return FLocalVariableManager::RemoveLocalVariable(Params);
    }
    // Custom events
    else if (CommandType == TEXT("add_custom_event"))
    {
        return FCustomEventManager::AddCustomEvent(Params);
    }
    else if (CommandType == TEXT("add_event_parameter"))
    {
        return FCustomEventManager::AddEventParameter(Params);
    }
    // Node layout
    else if (CommandType == TEXT("arrange_nodes_auto"))
    {
        return FNodeLayoutManager::ArrangeNodesAuto(Params);
    }
    else if (CommandType == TEXT("arrange_nodes_linear"))
    {
        return FNodeLayoutManager::ArrangeNodesLinear(Params);
    }
    else if (CommandType == TEXT("set_node_position"))
    {
        return FNodeLayoutManager::SetNodePosition(Params);
    }
