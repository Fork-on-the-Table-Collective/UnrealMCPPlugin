...
TSharedPtr<FJsonObject> FBlueprintNodeManager::AddNode(const TSharedPtr<FJsonObject>& Params)
{
	...
	{
		NewNode = FLoopNodeCreator::CreateForLoopNode(Graph, NodeParams);
	}
	else if (NodeType.Equals(TEXT("WhileLoop"), ESearchCase::IgnoreCase))
	{
		NewNode = FLoopNodeCreator::CreateWhileLoopNode(Graph, NodeParams);
	}
	else if (NodeType.Equals(TEXT("ForEachLoop"), ESearchCase::IgnoreCase))
	{
		NewNode = FLoopNodeCreator::CreateForEachLoopNode(Graph, NodeParams);
	}
	// Struct Nodes
	else if (NodeType.Equals(TEXT("MakeStruct"), ESearchCase::IgnoreCase))
	{
		NewNode = FStructNodeCreator::CreateMakeStructNode(Graph, NodeParams);
	}
	else if (NodeType.Equals(TEXT("BreakStruct"), ESearchCase::IgnoreCase))
	{
		NewNode = FStructNodeCreator::CreateBreakStructNode(Graph, NodeParams);
	}
